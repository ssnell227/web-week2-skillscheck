...
const filtered = myNumbers.filter(n => n > 100)
...
const total = myNumbers.reduce((arr, acc) => arr + acc)
...
myNumbers.forEach((number, index)=> myNumbersIndex.push(index))
...
const forTheLoveOfGeorge = notGeorge.map(item => 'George')
...
const enemies = people.filter(person => person.friend === false)
